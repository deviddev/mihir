...
<div id="cookie-consent-modal" class="overflow-y-auto">
    @if ($seekingConsent)
        ...
        <template x-teleport="body">
            <div id="cookie-consent-modal-body" x-data="{ open: @entangle('openConsentModal') }" x-show="open"
                x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 -translate-x-full" x-transition:enter-end="opacity-100 translate-x-0"
                x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 translate-x-0"
                x-transition:leave-end="opacity-0 -translate-x-full" class="fixed bottom-0 left-0 w-full">
                <div class="flex z-[51] w-full h-20 items-center bg-yellow-900">
                    <div class="flex px-4 space-x-4 items-center">
                        ...
                        <div wire:click="giveConsent" class="{{ $link }}">
                            <span wire:loading.remove class="transition">{{ __('cookie-consent::text.yes') }}</span>
                            <span wire:loading
                                class="animate-pulse transition">{{ __('cookie-consent::text.thinking') }}</span>
                        </div>
                        <div wire:click="refuseConsent" class="{{ $link }}">
                            <span wire:loading.remove class="transition">{{ __('cookie-consent::text.no') }}</span>
                            <span wire:loading
                                class="animate-pulse transition">{{ __('cookie-consent::text.thinking') }}</span>
                        </div>
                        <div wire:click="toggleCookieModal" @click="$dispatch('hideScroll')"
                            class="{{ $link }}">
                            <span wire:loading.remove
                                class="transition">{{ __('cookie-consent::text.learn_more') }}</span>
                            <span wire:loading
                                class="animate-pulse transition">{{ __('cookie-consent::text.thinking') }}</span>
                        </div>
                    </div>
        ...
        <!-- Cookie policy modal -->
        <div x-data="{ open: @entangle('openCookieModal') }" x-show="open" x-transition:enter="transition ease-out duration-300"
            ...
            <div @click.outside="$wire.toggleCookieModal();$dispatch('hideScroll')"

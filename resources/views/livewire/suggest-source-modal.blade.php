<dialog wire:ignore.self class="modal cursor-auto" x-data x-on:open-source-suggestions-modal.window="$el.showModal()"
    ...
    <div class="modal-box py-8 dark:bg-black dark:border-2 dark:border-stone-800 space-y-4">
        ...
        <form wire:submit="submit" x-show="!$wire.isSubmitted">
            <div>
                <label class="form-control w-full">
                    ...
                    <input autofocus wire:model="url" type="text" placeholder="telex.hu"
        ...
        <div class="alert text-sm items-start bg-primary dark:bg-stone-800 text-white dark:text-stone-300 dark:border-stone-800"
            ...
            <span>{{ __('misc.we_have_received_your_suggestion') }} <br>
                {{ __('misc.thank_you_for_your_contribution') }}</span>
        </div>
    </div>

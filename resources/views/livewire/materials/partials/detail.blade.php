<div class="space-y-8">
    ...
    <div class="flex justify-between items-center">
        <div class="flex items-center gap-x-2 lg:gap-x-6">
            <button class="inline-flex items-center gap-x-1" x-data="likeMaterial(
                '{{ $material->slug }}',
                @js($this->isLiked),
                @js(auth()->check()),
                {{ $this->likesCount }}
            )" x-on:click="toggleLike">
                <x-heroicon-o-hand-thumb-up x-cloak class="inline-flex lg:size-8 size-6 hover:stroke-primary"
                    x-bind:class="{
                        'stroke-primary fill-primary': isLiked,
                        'stroke-stone-800 dark:stroke-stone-300': !isLiked
                    }" />
                <span class="opacity-70" x-text="likesCount > 0 ? likesCount : null">
                </span>
            </button>
            <button class="inline-flex" x-data="dislikeMaterial(
                '{{ $material->slug }}',
                @js($this->isDisliked),
                @js(auth()->check()),
            )" x-on:click="toggleDislike">
                <x-heroicon-o-hand-thumb-down x-cloak class="inline-flex lg:size-8 size-6 hover:stroke-primary"
                    x-bind:class="{
                        'stroke-primary fill-primary': isDisliked,
                        'stroke-stone-800 dark:stroke-stone-300': !isDisliked
                    }" />
            </button>
            <button class="inline-flex" x-data="bookmarkMaterial(
                '{{ $material->slug }}',
                @js($this->isBookmarked),
                @js(auth()->check()),
            )" x-on:click="toggleBookmark">
                <x-heroicon-o-bookmark x-cloak class="inline-flex lg:size-8 size-6 hover:stroke-primary"
                    x-bind:class="{

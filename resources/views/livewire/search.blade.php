<div x-data="{
    ...
    <label
        class="relative input input-bordered flex items-center gap-2 bg-stone-50 dark:bg-stone-900 border-stone-50 dark:border-stone-900 focus-within:border-stone-200 dark:focus-within:border-stone-800 !outline-none">
        <input type="text" class="grow" placeholder="{{ __('misc.search') }}" wire:model.live.debounce.500ms="query"
        ...
        @if (filled($query))
            <div class="absolute inset-x-0 z-10 top-full max-h-fit overflow-y-auto mt-2 rounded-btn bg-stone-50 dark:bg-stone-900 border border-stone-200 dark:border-stone-800"
                x-bind:class="{ 'hidden': !isAutoCompleteVisible }">
                @forelse ($materials as $materialItem)
                    <div class="flex gap-x-4 items-center p-3 lg:text-sm text-xs cursor-pointer border-b border-b-stone-200 dark:border-b-stone-950 hover:bg-stone-100 dark:hover:bg-stone-950"
                        x-on:click.prevent="() => {
                            $dispatch('open-material-modal', {
                        ...
                        <figure class="flex max-lg:hidden w-2/12">
                            <img loading="lazy" src="{{ $materialItem->thumbnail }}"
                                class="h-10 w-full object-cover rounded">
                        </figure>
                        <div class="w-full">
                            <div class="font-bold line-clamp-1" title="{!! $materialItem->title !!}">
                                {!! $materialItem->title !!}
                            </div>
